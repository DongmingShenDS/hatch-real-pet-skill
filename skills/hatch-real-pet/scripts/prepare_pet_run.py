...
REAL_PET_STYLE = (
    "Real Codex pet cutout style: realistic reference-faithful full-body pet, "
    "natural proportions, real fur/skin/feather/material texture, preserved markings, "
    "preserved face and expression language, preserved accessory placement, and crisp "
    "clean cutout edges suitable for chroma-key extraction. Keep the pet compact and "
    "readable inside a 192x208 cell while retaining realistic color and texture. "
    "Avoid pixel art, chibi proportions, cartoon mascot redesign, anime style, toy-like "
    "3D rendering, vector logo treatment, sticker treatment, dark outline overlays, "
    "stepped pixel edges, flat cel shading, generic stock-animal drift, scenery, floor "
    "contact, cast shadows, soft shadows, glow, blur, and loose flyaway edge wisps."
)
...
def infer_description(args: argparse.Namespace, reference_paths: list[Path]) -> str:
    ...
    if args.pet_notes.strip():
        return sentence(f"A realistic Codex-compatible pet: {args.pet_notes}")
    if reference_paths:
        return "A realistic Codex-compatible pet based on the provided reference image."
    return "A realistic original Codex-compatible pet ready for animation."
...
def infer_pet_notes(args: argparse.Namespace, reference_paths: list[Path]) -> str:
    ...
    return "a realistic original Codex-compatible pet"


def default_output_dir(pet_id: str) -> Path:
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    return Path.cwd() / "output" / "hatch-real-pet" / f"{pet_id}-{timestamp}"
...
def resolved_style_notes(raw_style_notes: str) -> str:
    ...
    if not raw_style_notes:
        return REAL_PET_STYLE
    return f"{REAL_PET_STYLE} Additional user style notes: {raw_style_notes}."
...
def base_pet_prompt(args: argparse.Namespace) -> str:
    ...
    return f"""Create a single clean realistic cutout reference image for a Codex app real pet named {args.display_name}.
...
def row_prompt(
    ...
    return f"""Create a single horizontal realistic cutout sprite strip for the Codex app real pet `{args.pet_id}` in the state `{state}`.
...
- Do not use {chroma_key}, pure {chroma_name}, or colors close to that chroma key in the pet, props, highlights, shadows, motion marks, dust, landing marks, edges, or effects.
...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-concept-only",
        action="store_true",
        help="Allow a no-reference realistic pet run. Use only when the user explicitly approved concept-only generation.",
    )
    ...
    if not raw_reference_paths and not args.allow_concept_only:
        raise SystemExit(
            "hatch-real-pet requires at least one --reference image; pass --allow-concept-only only when the user explicitly approved a concept-only realistic pet"
        )
    ...
    request = {
        ...
        "reference_policy": "reference-first" if copied_refs else "explicit concept-only",
        "real_pet_style": REAL_PET_STYLE,
